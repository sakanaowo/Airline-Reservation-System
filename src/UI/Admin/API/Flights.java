...
public class Flights {
    ...
    public static String[] cities = {"VCL", "THD", "VDH", "DIN", "TBB", "PXU", "BMV", "VKG", "CAH", "VCS",
    ...
    public static boolean insertFlight(Timestamp departureTime, int planeID,
        ...
        try (Connection connection = DriverManager.getConnection(
                CommonConstants.DB_URL, CommonConstants.DB_USERNAME, CommonConstants.DB_PASSWORD); PreparedStatement checkStmt = connection.prepareStatement(checkQuery); PreparedStatement adminIDStmt = connection.prepareStatement(
                "SELECT AdminID FROM " + CommonConstants.DB_ADMIN_TABLE + " WHERE AdminName = ?"); PreparedStatement departureAirportStmt = connection.prepareStatement(
                        ...
                        + "JOIN " + CommonConstants.DB_PLANES_TABLE + " p ON p.LocationID = a.AirportID "); PreparedStatement arrivalAirportStmt = connection.prepareStatement(
                        "SELECT AirportID, AirportCode FROM " + CommonConstants.DB_AIRPORTS_TABLE + " WHERE City = ?"); PreparedStatement insertFlightStmt = connection.prepareStatement(insertFlightSQL)) {
            ...
            for (int i = 0; i < cities.length; i++) {
                if (departureAirportCode.equals(cities[i])) {
                    x = i;
                }
            }
            for (int i = 0; i < cities.length; i++) {
                if (arrivalAirportCode.equals(cities[i])) {
                    y = i;
                }
            }
    ...
    public static ArrayList<ArrayList<Object>> viewFlight(String departureTime, String arrivalTime,
        ...
        try (Connection connection = DriverManager.getConnection(
                CommonConstants.DB_URL, CommonConstants.DB_USERNAME, CommonConstants.DB_PASSWORD); PreparedStatement viewFlightsStmt = connection.prepareStatement(viewFlightSQL)) {
    ...
    public static boolean deleteFlight(int flightID) {
        ...
        try (Connection connection = DriverManager.getConnection(
                CommonConstants.DB_URL, CommonConstants.DB_USERNAME, CommonConstants.DB_PASSWORD); PreparedStatement deleteFlightStmt = connection.prepareStatement(deleteFlightSQL)) {
